...
num_gathers=1
batch_size=2048
description="correctness_test_3"
numactl_use=1
locality="uniform"
numa_cmd=""
...
do
    ...
    do
        $numa_cmd python $pdb_cmd ../dlrm/dlrm_s_pytorch.py $model_cmd --emb-scale=$emb_scale --num-batches=$iterations --mini-batch-size=$batch_size --use-gpu --num-indices-per-lookup=$num_gathers --num-indices-per-lookup-fixed=True --dpsgd-mode=$training_mode  --disable-poisson-sampling --system=$system --description=$description --path-lazydp=$PATH_LAZYDP --locality=$locality --path-model-weight=$PATH_MODEL_WEIGHT --is-debugging --debugging-type=without_noise_clipping
    done
done

python check_correctness_3.py --path-model-weight=$PATH_MODEL_WEIGHT